...
export const ConnectWallet = ({
	...
	const { chain, isConnected, connector: activeConnector, status } = useAccount();
	const { connect, connectors, isLoading } = useConnect();
	const { switchChain } = useSwitchChain();
	const [selectedConnector, setSelectedConnector] = useState<ConnectorProps | undefined>(
	...
	const amountToPay = useMemo(() => parseUnits(amount.toString(), tokenDecimal), [amount, tokenDecimal]);
	...
	return (
		<div className="flex flex-col gap-8">
			<p className="text-center text-sm text-body">
				By making payment you acknowledge that you have read and understand the{" "}
				<a
