			...
			{connectors.map((connector: ConnectorProps) => {
				...
				return (
					<button
						...
						className={`flex items-center justify-between rounded-2xl border border-[#DFDFE6] p-1 px-4 py-3 text-left hover:border-blue-darkest
						hover:!border-opacity-30 disabled:cursor-not-allowed disabled:opacity-50 ${
						isActive
					...
					>
						<span className="flex w-full items-center gap-2">
							...
							{isLoading && selectedConnector?.id === connector?.id && (
						...
						<span>{logo != null && <img src={logo} width={20} height={20} alt="" />}</span>
					</button>
				);
			})}
		</div>
